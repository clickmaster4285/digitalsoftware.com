...
export default function Home() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set(globeWrapRef.current, { scale: 1, yPercent: 50, xPercent: 18 });
      gsap.set(screen1Ref.current, { opacity: 1, pointerEvents: "auto" });
      gsap.set(screen3Ref.current, { opacity: 0, y: 40, pointerEvents: "none" });
      ...
      tl.to(screen1Ref.current, { opacity: 0, y: -60, duration: 0.4 }, 0)
        .to(globeWrapRef.current, { yPercent: 0, scale: 2, xPercent: 0, duration: 0.6 }, 0.1)
        .to(bgRef.current, { backgroundColor: "#111111", duration: 0.5 }, 0.1)
        ...
        .to(bgRef.current, { backgroundColor: "#FFFFFF", duration: 0.2 }, 0.85)
        ...
        .to(screen3Ref.current, {
          ...
          ease: "power2.out",
          onStart: () => {
            if (screen3Ref.current) screen3Ref.current.style.pointerEvents = "auto";
          },
        }, 0.95);
    }, sectionRef);
      ...
      >
        ...
        <div ref={networkRef} className="pointer-events-none absolute inset-0">
          {PORTRAITS.map((p, i) => (
            <div
              ...
              className="absolute h-10 w-10 overflow-hidden rounded-full border-2 border-white shadow-xl -translate-x-1/2 -translate-y-1/2"
        ...
        >
          <div className="max-w-[90rem] lg:pr-[28rem]">
            ...
            <h1 className="text-5xl leading-[1.05] font-bold sm:text-6xl lg:text-7xl" style={{ fontFamily: "'Instrument Serif', Georgia, serif" }}>
              From <span className="text-[#FF2E86]">strategy</span> to <span className="text-[#FF2E86]">execution</span> our team is ready to help.{" "}
              <span className="text-[#FF2E86]">Scroll down</span> and fill out the form below to start the{" "}
              <span className="text-[#FF2E86]">conversation</span>. We would love to hear from{" "}
              <span className="relative inline-block italic" style={{ fontFamily: "'Instrument Serif', Georgia, serif", fontSize: "1.15em" }}>
                ...
                <svg className="absolute -inset-x-6 -inset-y-3 h-[140%] w-[120%]" viewBox="0 0 200 100" preserveAspectRatio="none">
                  <ellipse cx="100" cy="50" rx="92" ry="42" fill="none" stroke="#FF2E86" strokeWidth="2" />
                </svg>
              </span>
            </h1>
            <button className="mt-10 inline-flex items-center gap-2 rounded-full bg-white px-7 py-3.5 text-sm font-medium text-black transition-transform hover:scale-105 hover:bg-[#FF2E86] hover:text-white">
              Get in touch
            </button>
          </div>
        </div>
          ...
          <div className="mt-8 sm:mt-12 w-full max-w-xl mx-auto lg:mx-0">
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
              <div className="grid gap-6 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-gray-900 shadow-sm focus:border-black focus:ring-2 focus:ring-black/10 outline-none"
                  />
              ...
              <button
                ...
                className="w-full rounded-xl bg-black px-6 py-4 text-white font-medium transition hover:bg-gray-900 active:scale-[0.99] disabled:opacity-50"
              >
