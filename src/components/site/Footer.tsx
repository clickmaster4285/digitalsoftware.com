...
export default function Footer() {
  ...
  return (
    <footer
      ref={footerRef}
      className="relative overflow-hidden bg-[#0a0a0a] text-neutral-200 border-t border-neutral-800 pt-16 pb-12"
    >
      <div className="relative mx-auto max-w-[1400px] px-6 md:px-10">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-12">
          ...
          <div className="md:col-span-5 footer-item">
            ...
            <div className="mt-6 space-y-2 text-[17px]">
              <a
                ...
                className="group flex items-center gap-1 hover:text-[#FF2E86] transition-colors"
              >
          ...
          {/* Navigation */}
          <div className="md:col-span-4 grid grid-cols-2 gap-8 footer-item">
            <ul className="space-y-3 text-[15px]">
              {navCol1.map((item) => (
                <li key={item}>
                  <Link
                    href={`#${item.toLowerCase()}`}
                    className="group flex items-center gap-1 hover:text-[#FF2E86] hover:text-base hover:font-medium transition-all duration-200"
                  >
                    {item}
                    <ArrowUpRight className="w-4 h-4 opacity-0 -translate-y-1 translate-x-1 group-hover:opacity-100 group-hover:translate-y-0 group-hover:translate-x-0 transition-all duration-200" />
                  </Link>
          ...
          <div className="md:col-span-3 footer-item">
            <p className="text-xs uppercase tracking-[2px] text-neutral-500 mb-4">OFFICES</p>
            <div className="space-y-6 text-sm">
              {offices.map((o) => (
                <div key={o.city}>
                  <div className="font-medium text-neutral-100 hover:text-[#FF2E86] ">{o.city}</div>
                  <a
                    href={`https://maps.google.com/?q=${encodeURIComponent(`${o.line1} ${o.line2}`)}`}
                    target="_blank"
                    rel="noreferrer"
                    className="group flex items-start gap-1 text-neutral-400 hover:text-neutral-200 transition-colors mt-1 leading-snug"
                  >
                    <span>
                      {o.line1}
                      <br />
        ...
        {/* Bottom Bar */}
        <div className="mt-16 pt-8 border-t border-neutral-800 flex flex-col md:flex-row items-center justify-between gap-6 text-sm text-neutral-400 footer-item">
          <div>© 2026 ClickMasters Digital Marketing Agency</div>

          <div className="flex items-center gap-8">
            <div className="flex gap-6">
              <Link href="#privacy" className="group flex items-center gap-1 hover:text-neutral-200 transition-colors">
                Privacy
              ...
              <Link href="#terms" className="group flex items-center gap-1 hover:text-neutral-200 transition-colors">
                Terms
                <ArrowUpRight className="w-3.5 h-3.5 opacity-0 -translate-y-1 translate-x-1 group-hover:opacity-100 group-hover:translate-y-0 group-hover:translate-x-0 transition-all duration-200" />
              </Link>
              <Link href="#sitemap" className="group flex items-center gap-1 hover:text-neutral-200 transition-colors">
                Sitemap
                <ArrowUpRight className="w-3.5 h-3.5 opacity-0 -translate-y-1 translate-x-1 group-hover:opacity-100 group-hover:translate-y-0 group-hover:translate-x-0 transition-all duration-200" />
              </Link>
            </div>
...
function SocialIcon({ label, path }: { label: string; path: string }) {
  return (
    <a
      href="#"
      aria-label={label}
      className="text-neutral-400 hover:text-[#FF2E86] transition-all duration-200 hover:scale-110"
    >
